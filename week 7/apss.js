...
function handleTransaction(type) {
  ...
  const balanceElement = document.getElementById(`${cryptoSelect}-balance`);

  let balance = parseFloat(balanceElement.textContent);

  if (type === "buy") {
    balance += amount / prices[cryptoSelect];
  } else if (type === "sell" && balance * prices[cryptoSelect] >= amount) {
    balance -= amount / prices[cryptoSelect];
  } else {
  ...
  document.getElementById("status-message").textContent = `Je hebt €${amount.toFixed(2)} in ${cryptoSelect.charAt(0).toUpperCase() + cryptoSelect.slice(1)} geïnvesteerd.`;
}
